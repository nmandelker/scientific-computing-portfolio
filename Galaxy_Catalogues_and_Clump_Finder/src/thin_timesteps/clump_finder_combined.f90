...
!!! Parameters used in the code, which can be varied to change performance, memory usage, definitions, implimentations, etc
	...
	logical,parameter :: h_equals_r = .false.      			!!! Set Hd = Rd for clump finder
	real(4),parameter :: disc_dim = 1.5_4				!!! Region searched for clumps: r' < disc_dim * Rd, |z'| < disc_dim * Hd
	real(4),parameter :: min_box_size = 3.0_4			!!! Region searched for clumps is at least +/- 3kpc in each direction.
	...
	logical,parameter :: dens_cell_thresh = .true.			!!! Use minimum (narrow) density to count cell - avoids having too many spurious cells in low density regions
!	real(4),parameter :: min_dens_cell = 0.03363_4 * 0.5_4		!!! Minimum allowable density of cell to count for clump finder. 0.5cm^{-3} in M_{sun} pc^{-3}
	real(4),parameter :: min_dens_cell = 0.03363_4			!!! Minimum allowable density of cell to count for clump finder. 1cm^{-3} in M_{sun} pc^{-3}
	...
	real(4),parameter :: tmax_track = 100.0_4			!!! Maximal time in Myr to look back in time for parent
	real(4),parameter :: Ndyn_track = 2.0_4				!!! Number of dynamical times (td_global) to look back in time when tracking clumps. (For unlimitted time, make 1.e9)
...
!	real(8),parameter :: min_mass_abs = 1.d6			!!! Baryonic mass threshold for clumps, in solar masses
	real(8),parameter :: min_mass_abs = (10.0_8)**(6.7_8)		!!! Baryonic mass threshold for clumps, in solar masses
	real(8),parameter :: min_dens_abs = 0.03363			!!! Baryonic volume density threshold for clumps, in solar mass per cubic parsec (0.03363 ~ 1cm^{-3})
...
!!!	---------------------------------
	...
	real(4),allocatable :: density_gas(:), temperature_gas(:), cell_size_gas(:), SNI_gas(:), SNII_gas(:), pot_gas(:)
...
!!!!!!!!!! Deallocated in 'deallocate_gas_stars_dm' !!!!!!!!!!
		...
		allocate( pot_gas(Ngas(snap)), stat=i )		!!! deallocated immediately !!!
		if(i.ne.0) then
			print *, 'error in allocation pot_gas. stat= ', i
			stop
		end if
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	subroutine allocate_stars(snap)
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine data_gas(filename, nstop)
		...
		DO WHILE (i .le. nstop)
			...
			& density_gas(i), temperature_gas(i), SNII_gas(i), SNI_gas(i), pot_gas(i)
			i = i + 1
		end do
 6		continue
		close (12)
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine data_stars(filename, nstop)
		...
		DO WHILE (i .le. nstop)
			...
			& Vystars(i), Vzstars(i), mass_stars(i), initial_mass_stars(i), age_stars(i), SNII_stars(i), SNI_stars(i)
			i = i + 1
		end do
 6		continue
		close (14)
		print *, i, nstop
		print *, maxval( mass_stars(1:nstop) / initial_mass_stars(1:nstop) )
		print *, minval( mass_stars(1:nstop) / initial_mass_stars(1:nstop) )
...
!!! This is a failsafe. Actually, everything should be deallocated by now. !!!
		...
		print *, 'deallocating gas stars and dm arrays'
		...
		if(allocated(pot_gas)) deallocate(pot_gas)
...
! 'trace_num' is the index of the tracer currently used, in order to place the output (surface densities, etc) in the correct file
		...
		if( use_narrow ) then
			...
			print *, 'SO WERE JUST GOING TO PUT THE RAW GID INTO THE SMOOTHED GRID'
...
!!!!!!!!!!!!!!!!!!!!!!!!! FOR CLUMPS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	real(8),allocatable :: rcar_clump(:,:), rcar_clump_disc(:,:), rcar_clump_density_peak(:,:), rcar_clump_disc_density_peak(:,:), vcyl_clump(:,:)
	real(8),allocatable :: group_rad(:,:), max_dens(:), eta_group(:)
	...
	integer,allocatable :: clump_id(:), exsitu(:), merger(:), new(:), ncell_2d(:), group_center_dens(:,:)
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine allocate_clump_prop2()
		...
		allocate( eta_group(ngroup), stat=i )
		if(i.ne.0) then
		...
		allocate( merger(ngroup), stat=i )
		if(i.ne.0) then
			print *, 'error in allocation merger. stat= ', i
			stop
		end if
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine deallocate_clump_prop()
		...
		print *, 'deallocating clump properties'
		...
		if(allocated(merger))              deallocate(merger)
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine clump_prop(gal, snapshot)
		...
		real(8) :: inert(3,3), inert2(3,3), inert_eigen(3), v(3,3), temp_var, temp_var2,  aI, bI, cI, same_stars_mass, mstar_in_clump(4), Vesc1, Vesc2, Vesc3, mass_temporary
		logical :: go_on
		...
		if(ngroup.gt.0) then
			...
			do i=1,ngroup
				...
				open(81,file=clump_stars_filename(i))
				...
				if(nstar_group(i,1).gt.0) then
					...
					do while(l .gt. 0)
						...
						& xstars(m), ystars(m), zstars(m), vxstars(m), vystars(m), vzstars(m), mass_stars(m), & 
						& initial_mass_stars(m), age_stars(m), SNII_stars(m)
						l = star_group(l,2)
						n = n + 1
					end do
			...
			do i=1,ngroup
				...
				if( clump_mass(i) .gt. 0.0_8 ) then
					...
					rcar_clump_disc(i,1) = sum( dens_group(ncell(i-1)+1:ncell(i)) * real(igroup(ncell(i-1)+1:ncell(i)) - nR/2, 8) ) * (real(res,8)/1000.0_8) / clump_mass(i)	!!! center of mass, disc frame, kpc
					rcar_clump_disc(i,2) = sum( dens_group(ncell(i-1)+1:ncell(i)) * real(jgroup(ncell(i-1)+1:ncell(i)) - nR/2, 8) ) * (real(res,8)/1000.0_8) / clump_mass(i)	!!! center of mass, disc frame, kpc
					rcar_clump_disc(i,3) = sum( dens_group(ncell(i-1)+1:ncell(i)) * real(kgroup(ncell(i-1)+1:ncell(i)) - nH/2, 8) ) * (real(res,8)/1000.0_8) / clump_mass(i)	!!! center of mass, disc frame, kpc
				else
				...
				group_center_dens(i,1) = igroup(ncell(i-1)+temp_loc(1))							!!! density peak, kpc
				group_center_dens(i,2) = jgroup(ncell(i-1)+temp_loc(1))							!!! density peak, kpc
				group_center_dens(i,3) = kgroup(ncell(i-1)+temp_loc(1))							!!! density peak, kpc
...
!				end if
			...
			do i=1,ngroup
				...
				inert(3,2) = inert(2,3)
				inert2(:,:) = inert(:,:)
				call jacobi(inert2, 3, 3, inert_eigen(:), v, nrot)
	
				!!! Order Inertia Tensor Eigenvalues !!!
				temp_var = maxval(inert_eigen(:))
				temp_loc = maxloc(inert_eigen(:))
				inert_eigen(temp_loc(1)) = inert_eigen(1)
				inert_eigen(1) = temp_var
				temp_var = maxval(inert_eigen(2:3))
				temp_loc = maxloc(inert_eigen(2:3))
				inert_eigen(temp_loc(1)+1) = inert_eigen(2)
				inert_eigen(2) = temp_var
	
				!!! Axes assuming homogeneous ellipsoid !!!
				cI = sqrt( (2.5_8/clump_mass(i)) * (inert_eigen(2) - inert_eigen(1) + inert_eigen(3)) )
				bI = sqrt( (2.5_8/clump_mass(i)) * (inert_eigen(3) - inert_eigen(2) + inert_eigen(1)) )
				aI = sqrt( (2.5_8/clump_mass(i)) * (inert_eigen(1) - inert_eigen(3) + inert_eigen(2)) )
		
				inert_eigen(:) = inert_eigen(:) / sqrt(dot_product(inert_eigen(:), inert_eigen(:)))
				eta_group(i) = inert_eigen(3)/inert_eigen(1)
			...
			deallocate( igroup, jgroup, kgroup, dens_group, group_center_dens )
			...
			do i=1,ngroup
				...
				if( nstar_group(i,1) > 0 ) then
					open(83,file=clump_stars_filename(i))			!!! stars in clump
					...
					if( track_clumps ) then
						...
						if( snapshot .gt. 1 .and. clump_comment(i) .ne. ' bulge' .and. nstar_group(i,1) .ge. min_star_track ) then
							j = snapshot-1
							delt = 0.0_4
							do while( j .ge. 1 .and. 1000.0_4 * delt .lt. min(tmax_track, Ndyn_track * td_global) ) ! disc diameter crossing time for clump tracking
								...
								do k=1,ngroup_hist(j)
									if( nstar_hist(j,k) .ge. min_star_track ) then
										...
										same_stars_num = 0
										open(84,file=final_clump_stars_filename(j,k))	!!! stars in clump
...
!												end do
											...
											else ! Already found a parent --> We have a merger of more than 1 parent !
												if( clump_mass_hist(j,k) .gt. clump_mass_hist(for_merger(1),for_merger(2)) ) then	!!! If the new parent candidate is more massive than the previous !!!
													clump_id(i) = id_hist(j,k)
...
!													end do
													...
													if( for_merger(3) .eq. 0 .and. for_merger(4) .eq.0 ) then ! Is there already a merger candidate? !!!
														...
														if( clump_mass_hist(j,k) .gt. clump_mass_hist(for_merger(3),for_merger(4)) ) then	!!! Which secondary candidate is more massive? !!!
															for_merger(3) = j
															for_merger(4) = k
														end if
													end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			do i=1,ngroup
				...
				write(j,'(1x,i5.5,1x,f6.3,23(1x,es10.2),1x,i3,1x,i1,20(1x,es10.2))') clump_id(i), redshift(snapshot), group_rad(i,3), & 
				& clump_gas_mass(i), clump_star_mass(i), clump_mass(i), clump_gas_frac(i), clump_dm_frac(i), clump_gas_sig(i), clump_star_sig(i), &
				& clump_sig(i), clump_age(i), clump_gas_met(i), clump_star_met(i), clump_SFR(i), clump_Sig_SFR(i), clump_SSFR(i), clump_tau(i), &
				& clump_dist(i), clump_height(i), Rdisc(snapshot)*clump_dist(i), Hdisc(snapshot)*clump_height(i), mean_residual(i), eta_group(i), & 
				& clump_dm_dens(i) / dm_back(i), exsitu(i), merger(i), tff(i), td(i), td_global, Mgas_in(i,:), Mgas_out(i,:), & 
				& Mstars_in(i), Mstars_out(i), Mstars_formed(i), alpha_vir(i)

				write(k,'(1x,i5.5,1x,f6.3,23(1x,es10.2),1x,i3,1x,i1,20(1x,es10.2))') clump_id(i), redshift(snapshot), group_rad(i,3), & 
				& clump_gas_mass(i) / Mgas_disc(snapshot), clump_star_mass(i) / Mstar_disc(snapshot), clump_mass(i) / Mbar_disc(snapshot), & 
				& clump_gas_frac(i) / fgas_disc(snapshot), clump_dm_frac(i), clump_gas_sig(i) / sigma_gas_disc(snapshot), & 
				& clump_star_sig(i) / sigma_stars_disc(snapshot), clump_sig(i) / sigma_bar_disc(snapshot), clump_age(i) / age_disc(snapshot), & 
				& 10.0_8**( clump_gas_met(i) - metgas_disc(snapshot) ), 10.0_8**( clump_star_met(i) - metstars_disc(snapshot) ), &
				& clump_SFR(i) / SFR_disc(snapshot), clump_Sig_SFR(i) / Sig_SFR_disc(snapshot), clump_SSFR(i) / SSFR_disc(snapshot), & 
				& clump_tau(i) / tau_disc(snapshot), clump_dist(i), clump_height(i), Rdisc(snapshot) * clump_dist(i), & 
				& Hdisc(snapshot) * clump_height(i), mean_residual(i), eta_group(i), clump_dm_dens(i) / dm_back(i), exsitu(i), merger(i), tff(i), & 
				& td(i), td_global, Mgas_in(i,:), Mgas_out(i,:), Mstars_in(i), Mstars_out(i), Mstars_formed(i), alpha_vir(i)
			end do
			...
			do i=1,Ntracer
				m = 40+i
				write(m) ngroup, clump_id(1:ngroup), new(1:ngroup), exsitu(1:ngroup), rcar_clump_disc(1:ngroup,1), rcar_clump_disc(1:ngroup,2), group_rad(1:ngroup,3), clump_mass(1:ngroup), eta_group(1:ngroup)
			end do
			...
			do while(j.le.ngroup)
				...
				& comm, clump_id(i), clump_merger_hist(snapshot,i), j, new(i), rcar_clump(i,:), vcar_clump(i,:), group_rad(i,3), & 
				& rcar_clump_disc(i,:), vcyl_clump(i,:), clump_gas_mass(i), clump_star_mass(i), clump_dm_mass(i), & 
				& clump_mass(i) / Mbar_disc(snapshot), ncell(i)-ncell(i-1), nstar_group(i,1), clump_SFR(i), clump_age(i), clump_gas_met(i), &
			...
			write(19,'(9es12.4,13(1x,i3),24(1x,es12.4))') redshift(snapshot), Rdisc(snapshot), Hdisc(snapshot), Rdisc(snapshot) / Rvir(snapshot), &
				...
				& nbbulge(:), nbinsitu(:), nbexsitu_rot(:), nbexsitu_nonrot(:), mdisc_bulge(:), mdisc_insitu(:), mdisc_exsitu_rot(:), &
				& mdisc_exsitu_nonrot(:), SFRdisc_bulge(:), SFRdisc_insitu(:), SFRdisc_exsitu_rot(:), SFRdisc_exsitu_nonrot(:)
...
!!!!!!!!!! What simulations will we be looking at today? !!!!!!!!!!
	...
	do j=1,Nsimulation
		...
		write(filename,'(a,a,a)') '/mnt/NAS3/nirm/thin_time_slices/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_halo_cat.txt'
!		write(filename,'(a,a,a)') '/BIGDATA/nirm/thin_time_slices_temporary_copy/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_halo_cat.txt'
		open(16,file=filename,form='formatted')
		...
		write(filename,'(a,a,a)') '/mnt/NAS3/nirm/thin_time_slices/galaxy_inputs/',trim(gal_name(j)),'/Nmax.txt'
!		write(filename,'(a,a,a)') '/BIGDATA/nirm/thin_time_slices_temporary_copy/galaxy_inputs/',trim(gal_name(j)),'/Nmax.txt'
		open(20,file=filename,form='formatted')
		read(20,'(1x,i4)') Nsnapshot2
		...
		do i=1,Nsnapshot
			read(20,'(1x,f6.4,3(1x,i))') aexp(i), Ngas(i), Nstars(i), Ndm(i)
		end do
		close(20)
		...
		do i=1,Nsnapshot
			read(16,'(1x,f6.4,3(1x,es12.5))') aexp2, Rvir(i), Mvir(i), Vvir(i)
		...
		write(filename,'(a,a,a)') '/mnt/NAS3/nirm/thin_time_slices/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_disc_cat.txt'
!		write(filename,'(a,a,a)') '/BIGDATA/nirm/thin_time_slices_temporary_copy/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_disc_cat.txt'
		open(17,file=filename,form='formatted')
		read(17,'(1x,i4)') Nsnapshot2
		if( Nsnapshot2 .ne. Nsnapshot ) then
			print *, 'DATA Nsnapshot - error with catalogue files. Inconsistent number of snapshots per galaxy'
			print *, 'Nsnapshot',Nsnapshot,'Nsnapshot2',Nsnapshot2
			stop
		...
		do i=1,Nsnapshot
			read(17,'(1x,f6.4,10(1x,es12.5),2(1x,f7.3),7(1x,es12.5),2(1x,f7.3))') aexp2, rcom(1,i), rcom(2,i), rcom(3,i), vcom(1,i), vcom(2,i), vcom(3,i), &
		...
		print *, 'snap_i:', snap_i
		print *, 'snap_f:', snap_f
		write(path_name,'(a,a,a,a)') '/mnt/NAS3/nirm/thin_time_slices/post_treated_files/',trim(gal_name(j)),'/',trim(gal_name(j))
!		write(path_name,'(a,a,a,a)') '/BIGDATA/nirm/thin_time_slices_temporary_copy/post_treated_files/',trim(gal_name(j)),'/',trim(gal_name(j))
		...
		!!! The important thing is to get the 6 history arrays, including final_clump_stars_filename, and the insitustars array !!!
			do k=1,snap_i-1
				...
				write(snap_tag,'(a4,a1,f6.4,a4)') trim(Rvir_string),'a',aexp(k),'.dat'
				write(dm_file_name(k),'(a,a,a)') trim(path_name),'_D',trim(snap_tag)
				write(gas_file_name(k),'(a,a,a)') trim(path_name),'_GZp',trim(snap_tag)
				write(stars_file_name(k),'(a,a,a)') trim(path_name),'_SZi',trim(snap_tag)
				write(snap_tag2,'(a1,f6.4)') 'a',aexp(k)
				...
				call allocate_stars( k )
				deallocate( age_stars, mass_stars, vzstars, vystars, vxstars, SNI_stars, SNII_stars, initial_mass_stars )
...
!        		                print ('(3(1x,i3))'), ngroup_hist(k), nistemp, nestemp
					if( ngroup_hist(k) .gt. 0 ) then
						do i=1,ngroup_hist(k)
							...
							& comm_temp, id_hist(k,i), clump_merger_hist(k,i), m, h, xctemp, yctemp, zctemp, vcar_clump_temp(:), & 
							& rctemp, clump_cen_temp(:), vcyl_clump_temp(:), clump_gas_mass_temp, clump_star_mass_temp, &
							& clump_dm_mass_temp, clump_normalized_mass_temp, ncell_temp, nstar_hist(k,i), SFR_temp, age_temp, & 
							& met_temp(:), estemp
...
!							& clump_gas_mass_temp, clump_star_mass_temp, clump_dm_mass_temp, clump_normalized_mass_temp, ncell_temp, & 
!							& nstar_temp, SFR_temp, age_temp, met_temp(:), estemp

							clump_mass_hist(k,i) = clump_gas_mass_temp + clump_star_mass_temp
							write(final_clump_stars_filename(k,i),'(a,a,a,f6.4,a,i5.5,a)') './',trim(dirname),'/clump_stars/a',aexp(k),'_final_clump',id_hist(k,i),'.out'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Open files, make grids and find clumps !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
			...
			do i=1,Ntracer
				...
				print *, 'opening file num',m
				open(m,file=filename,form='unformatted')
			end do
			Nhigh_diff1 = 0
			Nhigh_diff2 = 0
			do i=1,Ntracer
			...
			if( ngroup .eq. 0 ) then
				...
				write(18,'(1x,f6.4,10(1x,es12.5),2(1x,f7.3),7(1x,es12.5),2(1x,f7.3))') aexp(k), rcom(:,k), vcom(:,k), Ldisc(:,k), Lmag(k), &
				...
				write(18,'(1x,f6.4,10(1x,e12.5),2(1x,f7.3),2(1x,i3),7(1x,es12.5),2(1x,f7.3))')  aexp(k), rcom(:,k), vcom(:,k), Ldisc(:,k), Lmag(k), &	!!! disc_cat2
			...
			print *, 'closing clump catalogues'
			close(22)
			close(21)
			print *, 'closing binary grid files'
			call deallocate_gas_stars_dm()
			call deallocate_primes()
			do i=1,Ntracer
				write(filename,'(a,a,a,a,a,a,a)') './',trim(dirname),'/binary_grid_outputs/face_on_surface_densities_',trim(tracer_name(i)),'_',&
					& trim(snap_tag2),'.bin'
				inquire(file=filename, exist=file_exist)
				if(file_exist) then
					m=40+i
					print *, 'm', m
					inquire(unit=m, OPENED=file_opened)
					if(file_opened) then
						print *, 'm', m
						close(m)
					end if
			...
			print *, 'closed binary grid files'
			print *, ''
		end do
		print *, 'done'
...
!___________________________________________________________________________________________________________________________________________________________________________________________________________________
	...
	subroutine open_files( sim_num )
		...
		if( appen(sim_num) .eq. 0 ) then
			...
			open(31,file=filename,form='formatted')
		else
			write(filename,'(a,a,a)') './',trim(dirname),'/galaxy_catalogue/Nir_disc_cat2.txt'
			open(18,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/galaxy_catalogue/disc_sizes_and_clumps_HcSp.out'
			open(19,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/galaxy_catalogue/too_large_grids.out'
			open(23,file=filename,STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/in_situ.out'
			open(26,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/ex_situ.out'
			open(27,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/bulge.out'
			open(28,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/normalized_in_situ.out'
			open(29,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/normalized_ex_situ.out'
			open(30,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')

			write(filename,'(a,a,a)') './',trim(dirname),'/Matlab_friendly_data/normalized_bulge.out'
			open(31,file=filename,form='formatted',STATUS='OLD',POSITION='APPEND')
		end if
