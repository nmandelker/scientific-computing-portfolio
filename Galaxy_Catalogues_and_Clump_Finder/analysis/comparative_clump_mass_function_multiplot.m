function comparative_clump_mass_function_multiplot(is2, norm_is2, is3, norm_is3, ... 
    norm, disc2, disc3, clump_dens_thresh, norm_lifetime_thresh, mass_thresh, max_mass_thresh, ...
    type, common)
...
if(max_mass_thresh(2)>10 & max_mass_thresh(1)<6)
    if(mass_thresh(2) <= 10)
        ...
        tit = strcat('\:',num2str(mass_thresh(2),'%3.1f'),'>{\rm log}(M_{\rm c})');
    else
        max_mass = 1e5;
        if(mass_thresh(1) >= 6)
            tit = '\:{\rm log}(M_{\rm c})';
...
if(max_mass_thresh(2) <= 10)
    ...
    tit = strcat('\:',num2str(max_mass_thresh(2),'%3.1f'),'>{\rm log}(M_{\rm c,\,max}\:)');
else
    max_max_mass = 1e5;
    if(max_mass_thresh(1) >= 6)
        tit = '\:{\rm log}(M_{\rm c,\,max}\:)';
%     else
%         tit = '\:';
    end
end
...
% ystr = {num2str(0.001,'%5.3f'),num2str(0.01,'%4.2f'),num2str(0.1,'%3.1f'),num2str(1,'%1.0f')};
estr = {'','','','','',''};
...
%     'position',[0.5 0.92 0]);%,'BackgroundColor',[.7 .9 .7]);
...
ngal_3 = length(disc3);
[ngal_2, ngal_3]
if(common==1 & ngal_2 ~= ngal_3)
    disp('inconsistent number of snapshots')
    disp('Make sure you input COMMON snapshots')
    return
end

clump2 = 0.001.*ones(3,length(bin));
clump3 = 0.001.*ones(3,length(bin));
for i=1:length(bin)
%     if(type==5 | type==6 | type==17)
        clump2(1,i) = length(find(log10(x2(:,m))>=bin(i)-dbin & log10(x2(:,m))<bin(i)+dbin & ...
            log10(is2(:,6))>=min_mass & log10(is2(:,6))<max_mass & ...
            log10(is2(:,49))>=min_max_mass & log10(is2(:,49))<max_max_mass));
...
%         clump3(1,i) = length(find(log10(x3(:,m))>=bin(i)-dbin & log10(x3(:,m))<bin(i)+dbin));
%     end
    clump2(2,i) = length(find(log10(x2(:,m))>=bin(i)-dbin & log10(x2(:,m))<bin(i)+dbin & ...
        is2(:,48)>=min_dens & is2(:,48)<max_dens & ...
        norm_is2(:,52)>1e-6 & ...
        log10(is2(:,6))>=min_mass & log10(is2(:,6))<max_mass & ...
        log10(is2(:,49))>=min_max_mass & log10(is2(:,49))<max_max_mass));
    clump3(2,i) = length(find(log10(x3(:,m))>=bin(i)-dbin & log10(x3(:,m))<bin(i)+dbin & ...
        ...
        log10(is3(:,6))>=min_mass & log10(is3(:,6))<max_mass & ...
        log10(is3(:,49))>=min_max_mass & log10(is3(:,49))<max_max_mass));
    clump2(3,i) = length(find(log10(x2(:,m))>=bin(i)-dbin & log10(x2(:,m))<bin(i)+dbin & ...
        is2(:,48)>=min_dens & is2(:,48)<max_dens & ...
        norm_is2(:,52)>=min_norm_lifetime & norm_is2(:,52)<max_norm_lifetime & ...
        log10(is2(:,6))>=min_mass & log10(is2(:,6))<max_mass & ...
        log10(is2(:,49))>=min_max_mass & log10(is2(:,49))<max_max_mass));
...
[sum(clump2(1,:)), sum(clump3(1,:)), sum(clump2(2,:)), sum(clump3(2,:))]
clump2(1,:) = clump2(1,:) ./ ngal_2;
clump3(1,:) = clump3(1,:) ./ ngal_3;
clump2(2,:) = max(0.001,clump2(2,:) ./ ngal_2);
clump3(2,:) = max(0.001,clump3(2,:) ./ ngal_3);
clump2(3,:) = max(0.001,clump2(3,:) ./ ngal_2);
clump3(3,:) = max(0.001,clump3(3,:) ./ ngal_3);

if(multi == 1)
    if(type~=7 & type~=8 & type ~=9 & min_norm_lifetime==20)
        l(1) = stairs(bin,clump2(1,:),'marker','none','linewidth',4,'color','b','linestyle',':','DisplayName','$NoRP,\:ZL+SL+LL$');
        l(4) = stairs(bin,clump3(1,:),'marker','none','linewidth',4,'color','r','linestyle',':','DisplayName','$RP,\:ZL+SL+LL$');%[.565,.22,.042]
        l(2) = stairs(bin,clump2(2,:),'marker','none','linewidth',3,'color','b','linestyle','--','DisplayName','$NoRP,\:SL+LL$');
        l(5) = stairs(bin,clump3(2,:),'marker','none','linewidth',3,'color','r','linestyle','--','DisplayName','$RP,\:SL+LL$');
        l(3) = stairs(bin,clump2(3,:),'marker','none','linewidth',3,'color','b','linestyle','-','DisplayName','$NoRP,\:LL$');
        l(6) = stairs(bin,clump3(3,:),'marker','none','linewidth',3,'color','r','linestyle','-','DisplayName','$RP,\:LL$');
        leg_pos1 = [14.0 15.0 0.20 0.20];
        leg_pos2 = [6.5 15.0 0.20 0.20];
    else
%         l(1) = stairs(bin,clump2(1,:),'marker','none','linewidth',3,'color','b','linestyle','--','DisplayName','$NoRP,\:Non\:ZL$');
%         l(2) = stairs(bin,clump3(1,:),'marker','none','linewidth',3,'color','r','linestyle','--','DisplayName','$RP,\:Non\:ZL$');
        l(1) = stairs(bin,clump2(3,:),'marker','none','linewidth',3,'color','b','linestyle','-','DisplayName',strcat('$NoRP,',tit));
        l(2) = stairs(bin,clump3(3,:),'marker','none','linewidth',3,'color','r','linestyle','-','DisplayName',strcat('$RP,',tit));%[0.8706 0.4902 0]
    end
    if(type==7)
    ...
    l(1) = stairs(bin,clump2(1,:),'marker','none','linewidth',3,'color','b','linestyle','-','DisplayName','$NoRP$');
    l(2) = stairs(bin,clump3(1,:),'marker','none','linewidth',3,'color','r','linestyle','-','DisplayName','$RP$');
...
if(type==7)
    ...
    % Create textbox
    annotation(gcf,'textbox',[0.14 0.72 0.09 0.0535],...
        'String','$SLCs$',...
        'Interpreter','Latex',...
        'FontSize',24,...
        'FontName','Times',...
...
mkdir('./Gen2_vs_Gen3_mass_functions/');
if(common==1)
    mkdir(strcat('./Gen2_vs_Gen3_mass_functions/common_snapshots/'));
    mkdir(strcat('./Gen2_vs_Gen3_mass_functions/common_snapshots/multiplot/'));
    dirname = strcat('./Gen2_vs_Gen3_mass_functions/common_snapshots/multiplot/');
else
    mkdir(strcat('./Gen2_vs_Gen3_mass_functions/all_snapshots/'));
    mkdir(strcat('./Gen2_vs_Gen3_mass_functions/all_snapshots/multiplot/'));
    dirname = strcat('./Gen2_vs_Gen3_mass_functions/all_snapshots/multiplot/');
end
if(type==0)
