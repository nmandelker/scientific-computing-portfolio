# Cold Stream Stability: One Research Program, One Physical Ingredient at a Time

**Analytic instability theory + numerical experiments I designed and ran
myself — an eleven-work research program built by adding one piece of
physics at a time, with a governing dimensionless number identified at
every step.**
...
students I supervised (D. Padnos, H. Aung, Z. Yao), and the program's
...
shredded by the Kelvin–Helmholtz instability (KHI) at the sharp interface
...
up: start from pure hydrodynamics, then admit one new physical ingredient at
a time — and at each step, identify the *dimensionless number* that
...
| 7 | Magnetic fields | plasma β | unpublished (complete; presented at conferences) |

This repository releases the program in stages. **Stage 1 (linear theory +
its simulation verification) is included below.** The cooling/TRML stage —
the pivotal one, with its full vertical slice of RAMSES patches, analysis
pipeline, and plotting — is in preparation, as are conference figures from
the unpublished magnetic-fields stage.
...
numerically solves the linear dispersion relations for KHI in planar-sheet,
planar-slab, and 3D-cylinder geometry, and the MATLAB layer that turns those
solutions into growth-rate maps, phase diagrams, mode structures, and
...
number M_b and density contrast δ. From Mandelker et al. 2016, Fig. 1.*
...
*Numerical solution of the slab dispersion relation at M_b = 1.5, δ = 100 —
a regime where the single interface is stable, yet the slab is unstable
through body modes. Growth rates (left) and phase velocities (right) for the
mode families. The solution grids behind this figure were produced by the
notebook in `linear_theory/`. From Mandelker et al. 2016, Fig. 4.*
...
first six unstable modes of the slab — surface modes cling to the
interfaces; body modes fill the interior with standing-wave patterns. From
Mandelker et al. 2016, Fig. 5.*

### Verification: seeding simulations with the theory's own eigenmodes
...
are included: the eigenmode-verification configuration and the production
configuration used for the paper's runs.
...
(M_b, δ) parameter space, for three stream geometries. This "can the stream
survive?" figure recurs throughout the paper series, updated as each new
physical ingredient is added. From Mandelker et al. 2016, Fig. 11.*
