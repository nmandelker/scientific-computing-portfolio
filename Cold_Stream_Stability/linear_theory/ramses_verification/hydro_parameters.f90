module hydro_parameters
  ...
  ! Kelvin Helmholtz
  integer,parameter::MAXPERTURB=50
  integer :: num_khi_eigenmodes
  real(dp) ::lambda_over_diam=0.0
  integer ::perturbed_var=3
  real(dp) ::normalized_amp=0.001
  real(dp),dimension(1:MAXPERTURB)::pert_normalized_amp=0.0
  real(dp),dimension(1:MAXPERTURB)::pert_lambda_over_diam=0.0
  complex(dp),dimension(1:MAXPERTURB)::pert_omega=(0,0)
  integer,dimension(1:MAXPERTURB)::pert_mode=1 !!! 1 for S modes, 2 for P modes
  integer :: smooth=2	!!! 0 for no smoothing, 1 for smoothing unperturbed profile only, 2 for smoothing perturbations as well
  logical :: flat=.true.
  integer :: sigma_smooth=0
  integer :: gauss_sigma_smooth=0
