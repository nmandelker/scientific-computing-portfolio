program amr2cell
  ...
  double precision totmass, n_hydro, ekk
  integer time_int
  character(30) t_num

  integer dim_count
  real size, posX, posY, posZ, velX, velY, velZ, dens, tem, colour
...
!  open(unit=20,file=nomfich,form='formatted')
  open(30,file="./output/ART_format_t"//TRIM(t_num)//".dat",form="unformatted",convert='big_endian')
...
!                    totmass = totmass + var(i,ind,1)*dx**3.0d0

!                    size = real(dx*boxlen)
!                    posX = real(x(i,1)*boxlen)
!                    posY = real((x(i,2)-0.5d0)*boxlen)
!                    posZ = real((x(i,3)-0.5d0)*boxlen)
!                    n_hydro = MAX(var(i,ind,1),smallr)
!                    velX = real(var(i,ind,2))/real(n_hydro)
!                    velY = real(var(i,ind,3))/real(n_hydro)
!                    velZ = real(var(i,ind,4))/real(n_hydro)
!                    colour = real(var(i,ind,6))/real(n_hydro)
!                    tem = var(i,ind,5)
!                    ekk = 0.0d0
!                    do dim_count =1,ndim
!                       ekk = ekk + 0.5d0*var(i,ind,dim_count+1)**2.0d0/n_hydro
!                    end do
!                    tem = (gamma-1.0)*(tem-ekk)
!                    tem = real(tem/n_hydro)
!                    dens = real(var(i,ind,1))
!                    write(30) size, posX, posY, posZ, velX, velY, velZ, dens, tem
                    write(30) sngl(dx), sngl(x(i,1)), sngl(x(i,2)), sngl(x(i,3)), & 
                         & sngl(var(i,ind,2)), sngl(var(i,ind,3)), sngl(var(i,ind,4)), sngl(var(i,ind,1)), sngl(var(i,ind,5)), sngl(var(i,ind,7))
                 end if
              end do
