...
turbulence, and built the measurement to test it. The result —
...
*The puzzle: the stability parameter Q_2comp measured at proto-clump
locations (solid) versus random disc locations (dashed) in three simulated
galaxies. Clumps form at Q values well above the classical instability
threshold (Q < 1, left of the purple line). From Inoue et al. 2016, Fig. 11.*
...
- The disc's ordered rotation and shear are ~10× larger than the turbulence
  and are neither compressive nor turbulent — they must be removed without
  distorting what remains.
- The disc is thin and stratified; the analysis must respect the slab
  geometry rather than assume isotropy.
- The velocity field lives on an adaptive mesh and must be resampled
  (cloud-in-cell, with conservation checks) onto uniform grids — up to
  ~2.7×10⁸ cells per snapshot — before FFTs (Intel MKL) are applicable.
...
`parameter_input_v11.dat` — including three strategies for removing the
...
decomposed field (velocity, momentum, or energy-weighted) — so the
...
differently today" — synthetic-data tests with injected ground truth —
...
the density map. From Mandelker et al. 2025, Fig. 3.*
...
simulation's history, from the decomposition pipeline. From Mandelker et al.
2025, Fig. 2.*
...
galaxy simulation. From Mandelker et al. 2025, Fig. 4.*
