...
!!! Parameters used in the code, which are read in from the file ART_decomposition_input.dat
	...
	real(8) :: G				!!! Units: Kpc * (km/sec)^2 * M_sun^(-1)
	real(8) :: pi

	real(4) :: res				!!! [pc], resolution of base uni grid
	real(4) :: decomp_weight		!!! vector which is decomposed is rho^{decomp_weight}*{\vec {v}}. If 0, pure velocity. If 1, momentum. If 0.5, Sqrt[Ek]*{\hat {v}}
	...
	real(8) :: type_2_FWHM			!!! FWHM of Gaussian used to smooth rotation curve before subtracting it
...
!!!	---------------------------------
	integer,allocatable :: tsnap(:)
	real(4),allocatable :: Rvir(:), rcom(:,:), vcom(:,:), Ldisc(:,:), Rdisc(:), Hdisc(:)
	real(8),allocatable :: Mvir(:), Vvir(:), Mgas_disc(:), Mstar_disc(:), M_Es_star_disc(:), SFR_disc(:), age_disc(:)
...
!!! Grid variables
	...
	real(4) :: xmax, zmax, disc_rad, disc_height, sigma_ratio(16)
...
!_____________________________________________________________________________________________________________________________________________________________________
	...
	! 'snapshot' is integer index of snapshot from main loop
		implicit none
		integer,intent(in) :: snapshot
		integer :: i, j, k, m, n, n1, ng_half, ng2_half, nR_half, nH_half, Nrot, ntot, Nvert
		integer :: lower_ind, upper_ind, lower_ind2, upper_ind2
		real(4) :: R_thresh, H_thresh
		...
		R_thresh = 1000.0_4 * grid_rad
		disc_rad = R_thresh
		H_thresh = 1000.0_4 * grid_height
		disc_height = H_thresh
		...
		write(21,'(2(1x,es12.5))') H_thresh, 1000.0_4 * Hdisc(snapshot)
		close(unit=21)
		...
		write(21,'(4(1x,es12.5))') R_thresh, 1000.0_4 * Rdisc(snapshot), 1300.0_4 * Rdisc(snapshot), 1000.0_4 * Hdisc(snapshot)
		close(unit=21)
!		write(filename,'(a,a,a,a)') trim(output_dirname),'/rotation_curves/',trim(snap_tag2),'_rotation_curves.out'
!		open(unit=22,file=filename)
!		do i=1,Nrot
!			write(22,'(23(1x,es12.5))') res*(sngl(i)-0.5_4), vrot(i,2), vrot(i,5), vrot(i,7), vrot(i,9), vrot(i,11), vrot(i,13), vrot(i,15), & 
!				& vrot(i,17), vrot(i,19), vrot(i,21), vrot(i,23), vrot(i,3), vrot(i,24:33)
!		end do
!		close(unit=22)
		rrot(:) = rrot(:) / res
...
!								vrot_interp = log10( vrot(m-1,2) ) + log10( rad/rrot(m-1) ) * log10( vrot(m,2)/vrot(m-1,2) ) / log10( rrot(m)/rrot(m-1) )
								vrot_interp = vrot(m-1,n) + ( rad - rrot(m-1) ) * ( vrot(m,n) - vrot(m-1,n) ) / ( rrot(m) - rrot(m-1) )
							else
!								vrot_interp = log10( vrot(m,2) )   + log10( rad/rrot(m) )   * log10( vrot(m+1,2)/vrot(m,2) ) / log10( rrot(m+1)/rrot(m) )
...
!_____________________________________________________________________________________________________________________________________________________________________
	...
	! Then creates 2D face on projections of cylindrical turbulence components and total turbulence magnitude
		...
		print *, 'outputting 2d pre-filter turbulence maps'
...
!_____________________________________________________________________________________________________________________________________________________________________
	...
	! snapshot is snapshot number from main loop, needed for io purposes
		...
		real(8) :: mnorm, nnorm, rad, rx, vx, vy, vx2, vy2, dens
		...
		call fft_decompose
		print *, 'Make 2D filtered turbulence maps'
		allocate( sigz(nR,nR,2), stat=i )
		if(i.ne.0) then
			print *, 'error in allocation of sigz. stat= ', i
			stop
		...
		allocate( sigt(nR,nR,2), stat=i )
		if(i.ne.0) then
			print *, 'error in allocation of sigt. stat= ', i
			stop
		end if
		...
		print *, 'outputting 2d post-filter turbulence maps'
		write(33) nR, nR, grid_rad, grid_rad, disc_rad, sigp(:,:,1), sigr(:,:,1), sigz(:,:,1), sigt(:,:,1), sigp(:,:,2), sigr(:,:,2), sigz(:,:,2), sigt(:,:,2)
		deallocate( sigz, sigr, sigp, sigt )

		if( fourier_decomp ) then
			print *, 'HOW MANY ZEROS 3D after decomposition: sigma_{cx,cy,cz,sx,sy,sz}'
			ntot = ngrid*ngrid*ngrid2
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigcx).lt.0.0_8),count(mask=abs(sigcx).eq.0.0_8),minval(abs(sigcx),mask=abs(sigcx).ne.0.0_8),maxval(abs(sigcx)),sum(abs(sigcx))/ntot
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigcy).lt.0.0_8),count(mask=abs(sigcy).eq.0.0_8),minval(abs(sigcy),mask=abs(sigcy).ne.0.0_8),maxval(abs(sigcy)),sum(abs(sigcy))/ntot
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigcz).lt.0.0_8),count(mask=abs(sigcz).eq.0.0_8),minval(abs(sigcz),mask=abs(sigcz).ne.0.0_8),maxval(abs(sigcz)),sum(abs(sigcz))/ntot
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigsx).lt.0.0_8),count(mask=abs(sigsx).eq.0.0_8),minval(abs(sigsx),mask=abs(sigsx).ne.0.0_8),maxval(abs(sigsx)),sum(abs(sigsx))/ntot
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigsy).lt.0.0_8),count(mask=abs(sigsy).eq.0.0_8),minval(abs(sigsy),mask=abs(sigsy).ne.0.0_8),maxval(abs(sigsy)),sum(abs(sigsy))/ntot
			print '(2(1x,i),3(1x,es12.5))', count(mask=abs(sigsz).lt.0.0_8),count(mask=abs(sigsz).eq.0.0_8),minval(abs(sigsz),mask=abs(sigsz).ne.0.0_8),maxval(abs(sigsz)),sum(abs(sigsz))/ntot
			print *, ''

			print *, '!!!!!!!!!!!!!!!!!!!'
			print *, 'CHECK DECOMPOSITION: V_{x,y,z}, max, min, mean'
			nnorm = real(ngrid*ngrid*ngrid2)
			mnorm = sum(density_grid)
			print '(4(1x,es12.5))', maxval(sigsx+sigcx-vx_grid),minval(sigsx+sigcx-vx_grid),sum(abs(sigsx+sigcx-vx_grid))/nnorm,sum(density_grid*abs(sigsx+sigcx-vx_grid))/mnorm
			print '(4(1x,es12.5))', maxval(sigsy+sigcy-vy_grid),minval(sigsy+sigcy-vy_grid),sum(abs(sigsy+sigcy-vy_grid))/nnorm,sum(density_grid*abs(sigsy+sigcy-vy_grid))/mnorm
			print '(4(1x,es12.5))', maxval(sigsz+sigcz-vz_grid),minval(sigsz+sigcz-vz_grid),sum(abs(sigsz+sigcz-vz_grid))/nnorm,sum(density_grid*abs(sigsz+sigcz-vz_grid))/mnorm
			print *, 'CHECK DECOMPOSITION INSIDE BUFFERED REGION:'
			nnorm = real(nR*nR*nH)
			mnorm = sum( density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) )
			print '(4(1x,es12.5))', maxval( sigsx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vx_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&       minval( sigsx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vx_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&     sum( abs( sigsx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vx_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ) ) / nnorm, &
					&   sum( density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) * &
					&           abs(sigsx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vx_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)) ) / mnorm
			print '(4(1x,es12.5))', maxval( sigsy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vy_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&       minval( sigsy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vy_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&     sum( abs( sigsy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vy_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ) ) / nnorm, &
					&   sum( density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) * &
					&           abs(sigsy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vy_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)) ) / mnorm
			print '(4(1x,es12.5))', maxval( sigsz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vz_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&       minval( sigsz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vz_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ), &
					&     sum( abs( sigsz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vz_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) ) ) / nnorm, &
					&   sum( density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) * &
					&           abs(sigsz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) + &
					&               sigcz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2) - &
					&             vz_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)) ) / mnorm
			print *, 'CHECK DECOMPOSITION AT CENTER:'
			print '(es12.5)', sigsx(ngrid/2,ngrid/2,ngrid2/2)+sigcx(ngrid/2,ngrid/2,ngrid2/2)-vx_grid(ngrid/2,ngrid/2,ngrid2/2)
			print '(es12.5)', sigsy(ngrid/2,ngrid/2,ngrid2/2)+sigcy(ngrid/2,ngrid/2,ngrid2/2)-vy_grid(ngrid/2,ngrid/2,ngrid2/2)
			print '(es12.5)', sigsz(ngrid/2,ngrid/2,ngrid2/2)+sigcz(ngrid/2,ngrid/2,ngrid2/2)-vz_grid(ngrid/2,ngrid/2,ngrid2/2)
			print *, 'DONE CHECKING DECOMPOSITION'
			print *, '!!!!!!!!!!!!!!!!!!!'

			vx_grid = 1.d-10
			vy_grid = 1.d-10
			vz_grid = 1.d-10
			vz_grid(2:(ngrid-1),2:(ngrid-1),2:(ngrid2-1)) =   sigcx(3:ngrid, 2:(ngrid-1), 2:(ngrid2-1)) - sigcx(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1)) + & 
									& sigcy(2:(ngrid-1), 3:ngrid, 2:(ngrid2-1)) - sigcy(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1)) + & 
									& sigcz(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2) - sigcz(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2))
			where( vz_grid .ge. 0.0_8 ) vx_grid = sigcx**2 + sigcy**2 + sigcz**2
			where( vz_grid .lt. 0.0_8 ) vy_grid = sigcx**2 + sigcy**2 + sigcz**2
		...
		deallocate( vx_grid, vy_grid, vz_grid, sigsy, sigsz )
		...
		allocate( dot_2d(nR,nR,2), stat=i )
		if(i.ne.0) then
			print *, 'error in allocation of dot_2d. stat= ', i
			stop
		end if
		do j=1,nR
			do i=1,nR
				...
				dot_2d(i,j,1)  = sum( density_grid(k,m,lower_ind2:upper_ind2) * sigcz(k,m,lower_ind2:upper_ind2) ) / dens		!!! ( km sec^{-1} )^2
				...
				dot_2d(i,j,2)  = sum( sigcz(k,m,lower_ind2:upper_ind2) ) / real(nH)							!!! ( km sec^{-1} )^2
			end do
		end do
		print *, 'HOW MANY ZEROS 2D: sigma_{c,s,dot;1,2}'
		ntot = nR*nR
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigc_2d(:,:,1).lt.0.0_8),count(mask=Sigc_2d(:,:,1).eq.0.0_8),minval(Sigc_2d(:,:,1),mask=Sigc_2d(:,:,1).ne.0.0_8),maxval(Sigc_2d(:,:,1)),sum(Sigc_2d(:,:,1))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigc_2d(:,:,2).lt.0.0_8),count(mask=Sigc_2d(:,:,2).eq.0.0_8),minval(Sigc_2d(:,:,2),mask=Sigc_2d(:,:,2).ne.0.0_8),maxval(Sigc_2d(:,:,2)),sum(Sigc_2d(:,:,2))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigs_2d(:,:,1).lt.0.0_8),count(mask=Sigs_2d(:,:,1).eq.0.0_8),minval(Sigs_2d(:,:,1),mask=Sigs_2d(:,:,1).ne.0.0_8),maxval(Sigs_2d(:,:,1)),sum(Sigs_2d(:,:,1))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=dot_2d(:,:,1).lt.0.0_8),count(mask=dot_2d(:,:,1).eq.0.0_8),minval(dot_2d(:,:,1),mask=dot_2d(:,:,1).ne.0.0_8),maxval(dot_2d(:,:,1)),sum(dot_2d(:,:,1))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigc_2d(:,:,3).lt.0.0_8),count(mask=Sigc_2d(:,:,3).eq.0.0_8),minval(Sigc_2d(:,:,3),mask=Sigc_2d(:,:,3).ne.0.0_8),maxval(Sigc_2d(:,:,3)),sum(Sigc_2d(:,:,3))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigc_2d(:,:,4).lt.0.0_8),count(mask=Sigc_2d(:,:,4).eq.0.0_8),minval(Sigc_2d(:,:,4),mask=Sigc_2d(:,:,4).ne.0.0_8),maxval(Sigc_2d(:,:,4)),sum(Sigc_2d(:,:,4))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=Sigs_2d(:,:,2).lt.0.0_8),count(mask=Sigs_2d(:,:,2).eq.0.0_8),minval(Sigs_2d(:,:,2),mask=Sigs_2d(:,:,2).ne.0.0_8),maxval(Sigs_2d(:,:,2)),sum(Sigs_2d(:,:,2))/ntot
		print '(2(1x,i),3(1x,es12.5))', count(mask=dot_2d(:,:,2).lt.0.0_8),count(mask=dot_2d(:,:,2).eq.0.0_8),minval(dot_2d(:,:,2),mask=dot_2d(:,:,2).ne.0.0_8),maxval(dot_2d(:,:,2)),sum(dot_2d(:,:,2))/ntot

	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		print *, 'file 1'
		print *, nR, nR, grid_rad, grid_rad, disc_rad
		...
		test = Sigs_2d(:,:,1)+Sigc_2d(:,:,1)+Sigc_2d(:,:,2)+dot_2d(:,:,1)
		Sigs_2d(:,:,1) = Sigs_2d(:,:,1) / test
		Sigc_2d(:,:,1) = Sigc_2d(:,:,1) / test
		Sigc_2d(:,:,2) = Sigc_2d(:,:,2) / test
		dot_2d(:,:,1)  = dot_2d(:,:,1)  / test
		write(36) nR, nR, grid_rad, grid_rad, disc_rad, Sigs_2d(:,:,1), Sigc_2d(:,:,1), Sigc_2d(:,:,2), dot_2d(:,:,1)
		Sigs_2d(:,:,1) = Sigs_2d(:,:,1) * test
		Sigc_2d(:,:,1) = Sigc_2d(:,:,1) * test
		Sigc_2d(:,:,2) = Sigc_2d(:,:,2) * test
		dot_2d(:,:,1)  = dot_2d(:,:,1)  * test
		...
		print *, 'file 4'
		test = Sigs_2d(:,:,2)+Sigc_2d(:,:,3)+Sigc_2d(:,:,4)+dot_2d(:,:,2)
		Sigs_2d(:,:,2) = Sigs_2d(:,:,2) / test
		Sigc_2d(:,:,3) = Sigc_2d(:,:,3) / test
		Sigc_2d(:,:,4) = Sigc_2d(:,:,4) / test
		dot_2d(:,:,2)  = dot_2d(:,:,2)  / test
		write(37) nR, nR, grid_rad, grid_rad, disc_rad, Sigs_2d(:,:,2), Sigc_2d(:,:,3), Sigc_2d(:,:,4), dot_2d(:,:,2)
		...
		nnorm = sum( sigcx + sigcy + sigsx + sigcz )
		mnorm = sum( density_grid * (sigcx + sigcy + sigsx + sigcz) )
		sigma_ratio(1) = sum( sigsx ) / nnorm
		sigma_ratio(2) = sum( sigcx ) / nnorm
		sigma_ratio(3) = sum( sigcy ) / nnorm
		sigma_ratio(4) = sum( sigcz ) / nnorm
		sigma_ratio(5) = sum( density_grid * sigsx ) / mnorm
		sigma_ratio(6) = sum( density_grid * sigcx ) / mnorm
		sigma_ratio(7) = sum( density_grid * sigcy ) / mnorm
		sigma_ratio(8) = sum( density_grid * sigcz ) / mnorm

		nnorm = sum(       sigcx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + & 
				&  sigcy(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + &
				&  sigsx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + &
				&  sigcz(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) )
		mnorm = sum(density_grid(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) * ( &
				&  sigcx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + & 
				&  sigcy(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + &
				&  sigsx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) + &
				&  sigcz(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) ) )
		sigma_ratio(9) = sum( sigsx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) ) / nnorm
		sigma_ratio(10) = sum( sigcx(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) ) / nnorm
		sigma_ratio(11) = sum( sigcy(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) ) / nnorm
		sigma_ratio(12) = sum( sigcz(lower_ind:upper_ind, lower_ind:upper_ind, lower_ind2:upper_ind2) ) / nnorm
		sigma_ratio(13) = sum(density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)*sigsx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2))/mnorm
		sigma_ratio(14) = sum(density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)*sigcx(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2))/mnorm
		sigma_ratio(15) = sum(density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)*sigcy(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2))/mnorm
		sigma_ratio(16) = sum(density_grid(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2)*sigcz(lower_ind:upper_ind,lower_ind:upper_ind,lower_ind2:upper_ind2))/mnorm

		print *, 'volume ratios, whole box'
		print '(4(1x,es12.5))', sigma_ratio(1), sigma_ratio(2), sigma_ratio(3), sigma_ratio(4)
		print *, 'mass ratios, whole box'
		print '(4(1x,es12.5))', sigma_ratio(5), sigma_ratio(6), sigma_ratio(7), sigma_ratio(8)
		print *, 'volume ratios, small box'
		print '(4(1x,es12.5))', sigma_ratio(9), sigma_ratio(10), sigma_ratio(11), sigma_ratio(12)
		print *, 'mass ratios, small box'
		print '(4(1x,es12.5))', sigma_ratio(13), sigma_ratio(14), sigma_ratio(15), sigma_ratio(16)
		deallocate( density_grid, sigcx, sigcy, sigcz, sigsx )

		write(20,'(1x,i4,16(1x,es12.5))') tsnap(snapshot), sigma_ratio(1:4), sigma_ratio(9:12), sigma_ratio(5:8), sigma_ratio(13:16)

		deallocate( Sigc_2d, Sigs_2d, dot_2d, test )
...
!_____________________________________________________________________________________________________________________________________________________________________
	...
	! decomposes turbulence into solenoidal and compressive modes in Fourier space
		...
		real(8),allocatable :: test(:,:,:), kvec(:), PS_out(:,:)
		complex(8),allocatable :: fft_ind(:), fft_inx(:), fft_iny(:), fft_inz(:)
		complex(8),allocatable :: Gcx_1d(:),  Gcy_1d(:), Gcz_1d(:), Gsx_1d(:),  Gsy_1d(:), Gsz_1d(:)
		complex(8) :: im, Gk
		real(8) :: L, L2, dk1, dk2, mnorm, normc, norms
		...
		allocate( fft_ind(Nxyz), fft_inx(Nxyz), fft_iny(Nxyz), fft_inz(Nxyz) )
		do m=1,ngrid2
			do j=1,ngrid
				do i=1,ngrid
					fft_ind( (m-1)*Nxy + (j-1)*ngrid + i ) = density_grid(i,j,m)
		...
		error = DftiComputeForward(fft_desc, fft_ind)
		...
		fft_ind = fft_ind / sqrt(real(Nxyz))
		...
		print '(a9,3(1x,es12.5))', 'PARCEVAL ', mnorm, sum(abs(fft_ind)**2), mnorm/sum(abs(fft_ind)**2)
...
!							fft_ind(n) = 0.0_8
		...
		error = DftiCreateDescriptor(fft_desc, DFTI_DOUBLE, DFTI_COMPLEX, 3, length )
		error = DftiCommitDescriptor(fft_desc)
		error = DftiComputeBackward(fft_desc, fft_ind)
		error = DftiFreeDescriptor(fft_desc)
		print '(6(1x,es12.5))', maxval(real(fft_ind)), minval(real(fft_ind)), sum(real(fft_ind)**2)/Nxyz, maxval(aimag(fft_ind)), minval(aimag(fft_ind)), sum(aimag(fft_ind)**2)/Nxyz

		print *, 'fft_inx'
		error = DftiCreateDescriptor(fft_desc, DFTI_DOUBLE, DFTI_COMPLEX, 3, length )
		error = DftiCommitDescriptor(fft_desc)
		error = DftiComputeBackward(fft_desc, fft_inx)
		error = DftiFreeDescriptor(fft_desc)
		print '(6(1x,es12.5))', maxval(real(fft_inx)), minval(real(fft_inx)), sum(real(fft_inx)**2)/Nxyz, maxval(aimag(fft_inx)), minval(aimag(fft_inx)), sum(aimag(fft_inx)**2)/Nxyz

		print *, 'fft_iny'
		error = DftiCreateDescriptor(fft_desc, DFTI_DOUBLE, DFTI_COMPLEX, 3, length )
		error = DftiCommitDescriptor(fft_desc)
		error = DftiComputeBackward(fft_desc, fft_iny)
		error = DftiFreeDescriptor(fft_desc)
		print '(6(1x,es12.5))', maxval(real(fft_iny)), minval(real(fft_iny)), sum(real(fft_iny)**2)/Nxyz, maxval(aimag(fft_iny)), minval(aimag(fft_iny)), sum(aimag(fft_iny)**2)/Nxyz

		print *, 'fft_inz'
		error = DftiCreateDescriptor(fft_desc, DFTI_DOUBLE, DFTI_COMPLEX, 3, length )
		error = DftiCommitDescriptor(fft_desc)
		error = DftiComputeBackward(fft_desc, fft_inz)
		error = DftiFreeDescriptor(fft_desc)
		print '(6(1x,es12.5))', maxval(real(fft_inz)), minval(real(fft_inz)), sum(real(fft_inz)**2)/Nxyz, maxval(aimag(fft_inz)), minval(aimag(fft_inz)), sum(aimag(fft_inz)**2)/Nxyz
		...
		do m=1,ngrid2
			do j=1,ngrid
				do i=1,ngrid
					density_grid(i,j,m) = real(fft_ind( (m-1)*Nxy + (j-1)*ngrid + i ))
					vx_grid(i,j,m) = real(fft_inx( (m-1)*Nxy + (j-1)*ngrid + i ))
					vy_grid(i,j,m) = real(fft_iny( (m-1)*Nxy + (j-1)*ngrid + i ))
					vz_grid(i,j,m) = real(fft_inz( (m-1)*Nxy + (j-1)*ngrid + i ))
				end do
			end do
		end do
		deallocate( fft_ind, fft_inx, fft_iny, fft_inz )
		...
		if( fourier_decomp ) then
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gcx_1d)), minval(real(Gcx_1d)), sum(real(Gcx_1d)**2)/Nxyz, maxval(aimag(Gcx_1d)), minval(aimag(Gcx_1d)),sum(aimag(Gcx_1d)**2)/Nxyz
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gcy_1d)), minval(real(Gcy_1d)), sum(real(Gcy_1d)**2)/Nxyz, maxval(aimag(Gcy_1d)), minval(aimag(Gcy_1d)),sum(aimag(Gcy_1d)**2)/Nxyz
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gcz_1d)), minval(real(Gcz_1d)), sum(real(Gcz_1d)**2)/Nxyz, maxval(aimag(Gcz_1d)), minval(aimag(Gcz_1d)),sum(aimag(Gcz_1d)**2)/Nxyz
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gsx_1d)), minval(real(Gsx_1d)), sum(real(Gsx_1d)**2)/Nxyz, maxval(aimag(Gsx_1d)), minval(aimag(Gsx_1d)),sum(aimag(Gsx_1d)**2)/Nxyz
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gsy_1d)), minval(real(Gsy_1d)), sum(real(Gsy_1d)**2)/Nxyz, maxval(aimag(Gsy_1d)), minval(aimag(Gsy_1d)),sum(aimag(Gsy_1d)**2)/Nxyz
			...
			error = DftiFreeDescriptor(fft_desc)
			print '(6(1x,es12.5))', maxval(real(Gsz_1d)), minval(real(Gsz_1d)), sum(real(Gsz_1d)**2)/Nxyz, maxval(aimag(Gsz_1d)), minval(aimag(Gsz_1d)),sum(aimag(Gsz_1d)**2)/Nxyz
			...
			allocate( sigcz(ngrid,ngrid,ngrid2), stat=i )
			if(i.ne.0) then
				print *, 'error in allocation of sigcz. stat= ', i
				stop
			end if
			...
			allocate( sigsy(ngrid,ngrid,ngrid2), stat=i )
			if(i.ne.0) then
				print *, 'error in allocation of sigsy. stat= ', i
				stop
			end if
			allocate( sigsz(ngrid,ngrid,ngrid2), stat=i )
			if(i.ne.0) then
				print *, 'error in allocation of sigsz. stat= ', i
				stop
			end if

			print *, 'making output real matrices'
			do m=1,ngrid2
				do j=1,ngrid
					do i=1,ngrid
						sigcx(i,j,m) = real(Gcx_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
						sigcy(i,j,m) = real(Gcy_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
						sigcz(i,j,m) = real(Gcz_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
						sigsx(i,j,m) = real(Gsx_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
						sigsy(i,j,m) = real(Gsy_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
						sigsz(i,j,m) = real(Gsz_1d( (m-1)*Nxy + (j-1)*ngrid + i ))
					end do
				end do
			end do
			deallocate( Gcx_1d, Gcy_1d, Gcz_1d, Gsx_1d, Gsy_1d, Gsz_1d )
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			...
			print *, '!!!!!'
			print *, 'TESTS'
			allocate( test(ngrid-2, ngrid-2, ngrid2-2) )
			m = (ngrid-2)*(ngrid-2)*(ngrid2-2)
			mnorm = sum( density_grid(2:(ngrid-1), 2:(ngrid-1), 2:(ngrid2-1)) )
			print *, 'divergence of solonoidal mode'
			test = 0.5_8 * (  sigsx(3:ngrid,     2:(ngrid-1), 2:(ngrid2-1)) - sigsx(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1)) + &
					& sigsy(2:(ngrid-1), 3:ngrid,     2:(ngrid2-1)) - sigsy(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1)) + &
					& sigsz(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2)     - sigsz(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2)) )
			print '(4(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid(2:(ngrid-1), 2:(ngrid-1), 2:(ngrid2-1)) * abs(test))/mnorm
			print *, 'divergence of compressive mode'
			test = 0.5_8*(    sigcx(3:ngrid,     2:(ngrid-1), 2:(ngrid2-1)) - sigcx(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1)) + &
					& sigcy(2:(ngrid-1), 3:ngrid,     2:(ngrid2-1)) - sigcy(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1)) + &
					& sigcz(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2)     - sigcz(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2)) )
			print '(4(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid(2:(ngrid-1), 2:(ngrid-1), 2:(ngrid2-1)) * abs(test))/mnorm
			print *, 'rotor of solonoidal mode'
			test = 0.5_8 * sqrt( &
					&  ( (sigsz(2:(ngrid-1), 3:ngrid, 2:(ngrid2-1)) - sigsz(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1))) - & 
					&    (sigsy(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2) - sigsy(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2))) )**2 + &
					&  ( (sigsx(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2) - sigsx(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2))) - & 
					&    (sigsz(3:ngrid, 2:(ngrid-1), 2:(ngrid2-1)) - sigsz(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1))) )**2 + &
					&  ( (sigsy(3:ngrid, 2:(ngrid-1), 2:(ngrid2-1)) - sigsy(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1))) - & 
					&    (sigsx(2:(ngrid-1), 3:ngrid, 2:(ngrid2-1)) - sigsx(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1))) )**2   &
					&  )
			print '(4(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid(2:(ngrid-1), 2:(ngrid-1), 2:(ngrid2-1)) * abs(test))/mnorm
			print *, 'rotor of compressive mode'
			test = 0.5_8 * sqrt( &
					&  ( (sigcz(2:(ngrid-1), 3:ngrid, 2:(ngrid2-1)) - sigcz(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1))) - & 
					&    (sigcy(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2) - sigcy(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2))) )**2 + &
					&  ( (sigcx(2:(ngrid-1), 2:(ngrid-1), 3:ngrid2) - sigcx(2:(ngrid-1), 2:(ngrid-1), 1:(ngrid2-2))) - & 
					&    (sigcz(3:ngrid, 2:(ngrid-1), 2:(ngrid2-1)) - sigcz(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1))) )**2 + &
					&  ( (sigcy(3:ngrid, 2:(ngrid-1), 2:(ngrid2-1)) - sigcy(1:(ngrid-2), 2:(ngrid-1), 2:(ngrid2-1))) - & 
					&    (sigcx(2:(ngrid-1), 3:ngrid, 2:(ngrid2-1)) - sigcx(2:(ngrid-1), 1:(ngrid-2), 2:(ngrid2-1))) )**2   &
					&  )
			print '(4(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid(2:(ngrid-1), 2:(ngrid-1), 2:(ngrid2-1)) * abs(test))/mnorm
			deallocate( test )

			allocate( test(ngrid, ngrid, ngrid2) )
			m = Nxyz
			mnorm = sum(density_grid)
			print *, 'difference magnitude from full field'
			test = sqrt( (vx_grid-sigsx-sigcx)**2 + (vy_grid-sigsy-sigcy)**2 + (vz_grid-sigsz-sigcz)**2 )
			print '(4(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid*abs(test))/mnorm

			print *, 'dot product'
			test = 2.0_8*( sigcx*sigsx + sigcy*sigsy + sigcz*sigsz )
			print '(6(1x,es12.5))', maxval(abs(test)), minval(abs(test)), sum(abs(test))/m, sum(density_grid*abs(test))/mnorm, sum(test)/m, sum(density_grid*test)/mnorm
			...
			print *, 'total power real space, volume: t, s/t c/t, dot/t'
			norms = sum(sigsx**2 + sigsy**2 + sigsz**2)
			normc = sum(sigcx**2 + sigcy**2 + sigcz**2)
			mnorm = sum(test)
			print '(4(1x,es12.5))', norms+normc+mnorm, norms/(norms+normc+mnorm), normc/(norms+normc+mnorm), mnorm/(norms+normc+mnorm)
			print *, 'total power real space, mass: t, s/t c/t, dot/t'
			norms = sum( density_grid * (sigsx**2 + sigsy**2 + sigsz**2) )
			normc = sum( density_grid * (sigcx**2 + sigcy**2 + sigcz**2) )
			mnorm = sum( density_grid * test )
			print '(4(1x,es12.5))', norms+normc+mnorm, norms/(norms+normc+mnorm), normc/(norms+normc+mnorm), mnorm/(norms+normc+mnorm)
			print *, 'average power ratios real space, volume: s/t c/t, dot/t'
			test = test + sigsx**2 + sigsy**2 + sigsz**2 + sigcx**2 + sigcy**2 + sigcz**2
			m = Nxyz
			print '(3(1x,es12.5))', sum((sigsx**2+sigsy**2+sigsz**2)/test)/m, sum((sigcx**2+sigcy**2+sigcz**2)/test)/m, sum(2.0_8*(sigcx*sigsx+sigcy*sigsy+sigcz*sigsz)/test)/m
			print *, 'average power ratios real space, mass: s/t c/t, dot/t'
			mnorm = sum(density_grid)
			print '(3(1x,es12.5))', sum(density_grid*(sigsx**2+sigsy**2+sigsz**2)/test)/mnorm, sum(density_grid*(sigcx**2+sigcy**2+sigcz**2)/test)/mnorm, & 
				& sum(density_grid*2.0_8*(sigcx*sigsx+sigcy*sigsy+sigcz*sigsz)/test)/mnorm
	...
	! decomposes turbulence into solenoidal and compressive modes locally in real space (using strain rate tensor)
		...
		if( local_version .eq. 1 ) then
			do m=2,ngrid2-1
				do j=2,ngrid-1
					do i=2,ngrid-1
						divx = (vx_grid(i+1,j,m) - vx_grid(i-1,j,m)) / 2.0_8
						divy = (vy_grid(i,j+1,m) - vy_grid(i,j-1,m)) / 2.0_8
						divz = (vz_grid(i,j,m+1) - vz_grid(i,j,m-1)) / 2.0_8
						...
						local_sigsx(1:3, 1:3, 1:3) = vx_grid(i-1:i+1, j-1:j+1, m-1:m+1) - vx_grid(i, j, m) - local_sigcx(1:3, 1:3, 1:3)
						local_sigsy(1:3, 1:3, 1:3) = vy_grid(i-1:i+1, j-1:j+1, m-1:m+1) - vy_grid(i, j, m) - local_sigcy(1:3, 1:3, 1:3)
						local_sigsz(1:3, 1:3, 1:3) = vz_grid(i-1:i+1, j-1:j+1, m-1:m+1) - vz_grid(i, j, m) - local_sigcz(1:3, 1:3, 1:3)
		...
		elseif( local_version .eq. 2 ) then
			do m=2,ngrid2-1
				do j=2,ngrid-1
					do i=2,ngrid-1
						divx = (vx_grid(i+1,j,m)-vx_grid(i-1,j,m) + vy_grid(i,j+1,m)-vy_grid(i,j-1,m) + vz_grid(i,j,m+1)-vz_grid(i,j,m-1)) / 6.0_8
						local_sigcx(1, 1:3, 1:3) = -1.0_8 * divx
						local_sigcx(3, 1:3, 1:3) = divx
...
!use omp_lib
	...
	integer :: i, j, k, l, m, Nsnapshot, Nsnapshot2, Nsimulation, tsnap2
	real(4) :: Rv2, buff_size
...
!!!!!!!!!! What simulations will we be looking at today? !!!!!!!!!!
	...
	do j=1,Nsimulation
		...
		Nsnapshot2 = 0
		!!!!!!!!!! Get array sizes !!!!!!!!!!
		write(filename,'(a,a,a)') '/BIGDATA/nirm/turbulence_decomposition/Bournaud_runs/galaxy_inputs/',trim(gal_name(j)),'/Nmax.txt'
		...
		do i=1,Nsnapshot
			read(16,'(3(1x,i))') Ngas(i), Nstars(i), Ndm(i)
		end do
		close(unit=16)
		print *, 'first and last Ngas, Nstars, Ndm'
		print *, Ngas(1), Nstars(1), Ndm(1)
		print *, Ngas(Nsnapshot), Nstars(Nsnapshot), Ndm(Nsnapshot)

		call allocate_global(Nsnapshot)
		!!!!!!!!!! Initialize the arrays per simulation !!!!!!!!!!
		write(filename,'(a,a,a)') '/BIGDATA/nirm/turbulence_decomposition/Bournaud_runs/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_halo_cat.txt'
		...
		do i=1,Nsnapshot
			read(17,'(1x,i4,3(1x,es12.5))') tsnap(i), Rvir(i), Mvir(i), Vvir(i)
		end do
		close(unit=17)

		write(filename,'(a,a,a)') '/BIGDATA/nirm/turbulence_decomposition/Bournaud_runs/galaxy_inputs/',trim(gal_name(j)),'/galaxy_catalogue/Nir_disc_cat.txt'
		...
		do i=1,Nsnapshot
			read(18,'(1x,i4,9(1x,e12.5),2(1x,f7.3),5(1x,es12.5))') tsnap2, rcom(1,i), rcom(2,i), rcom(3,i), &
			& vcom(1,i), vcom(2,i), vcom(3,i), Ldisc(1,i), Ldisc(2,i), Ldisc(3,i), Rdisc(i), Hdisc(i), &
			& Mgas_disc(i), Mstar_disc(i), M_Es_star_disc(i), SFR_disc(i), age_disc(i)
			if( tsnap2 .ne. tsnap(i) ) then
				print *, 'DATA tsnap - error with disc cat. Inconsistent time'
				print *, 'snapshot #',i,'tsnap',tsnap(i),'tsnap2',tsnap2
				stop
			end if
		end do
		close(unit=18)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Loop over all snapshots !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		write(path_name,'(a,a,a)') '/BIGDATA/nirm/turbulence_decomposition/Bournaud_runs/snaps/',trim(gal_name(j)),'/Bournaud'

		do k=1,Nsnapshot
			print *, gal_name(j), k, Nsnapshot
			Rv2 = 2.0_4 * Rvir(k)
			write(Rvir_string,'(i3.3,a1)') int(Rv2),'_'
			if( tsnap(k) .ge. 1000 ) then
				write(snap_tag,'(a4,a1,i4,a4)') trim(Rvir_string),'t',tsnap(k),'.dat'
				write(snap_tag2,'(a1,i4)') 't',tsnap(k)
			else
				write(snap_tag,'(a4,a1,i3,a4)') trim(Rvir_string),'t',tsnap(k),'.dat'
				write(snap_tag2,'(a1,i3)') 't',tsnap(k)
			end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Allocate gas data and compute grid size !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
			...
			write(filename,'(a,a,a,a)') trim(output_dirname),'/binary_grid_outputs/',trim(snap_tag2),'_pre_filter_turbulence.bin'
			open(unit=32,file=filename,form='unformatted')
			write(filename,'(a,a,a,a)') trim(output_dirname),'/binary_grid_outputs/',trim(snap_tag2),'_post_filter_turbulence.bin'
			open(unit=33,file=filename,form='unformatted')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		deallocate( tsnap, Rvir, Mvir, Vvir, rcom, vcom, Ldisc, Rdisc, Hdisc, Mgas_disc, Mstar_disc, M_Es_star_disc, SFR_disc, age_disc, gas_file_name )
		call close_files()
		print *, ''
	end do
	deallocate( gal_name )
...
!!!!!!!!!! Read in parameter file and check for inconsistencies !!!!!!!!!!
		...
		read(14,'(a24,E9.2E2)') s,G
		print '(a24,E9.2E2)', s,G
		...
		read(14,'(a24,F4.2)') s,Fixed_H
		print '(a24,F4.2)', s,Fixed_H
		read(14,'(a24,I1)') s,j
		print '(a24,I1)', s,j
		if(j .eq. 1) then
			use_old_H = .true.
...
!!!!!!!!!! Allocate global arrays per simulation !!!!!!!!!!
		...
		allocate( tsnap(N), stat=i )
		if(i.ne.0) then
			print *, 'error in allocation tsnap. stat= ', i
			stop
		end if
...
!_____________________________________________________________________________________________________________________________________________________________________
	...
	subroutine open_files()
		...
		if( decomp_type .eq. 2 ) then
			write(output_dirname,'(a,a,i4.4,a)') trim(output_dirname),'_',floor(type_2_FWHM),'pc'
		...
		write(output_dirname,'(a,a,f3.1)') trim(output_dirname),'/decomp_weight_',decomp_weight
		write(filename,'(a,a)') 'mkdir -p ',trim(output_dirname)
		call system(trim(filename))

		write(output_dirname,'(a,a,i3.3,a)') trim(output_dirname),'/grid_res_',floor(res),'pc'
		write(filename,'(a,a)') 'mkdir -p ',trim(output_dirname)
		call system(trim(filename))
